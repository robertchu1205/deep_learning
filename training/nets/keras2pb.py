...
parser.add_argument("-a", "--h5_amount", dest="h5_amount", help="Amount of h5 models to combine")
...
SAVED_PB_PATH = pb_dir + str(int(time.time())) 
...
def serving_parse_fn(image_string):
    ...
    # image_decode = tf.io.decode_image(image_string, channels=3, expand_animations=False)
    image_resized = tf.image.resize_images(image_decode,[int(w_h[1]),int(w_h[0])])
...
if int(args.h5_amount)==1:
    MODEL_H5 = dirs[0]
    K.set_learning_phase(0) 
    m = tf.keras.models.load_model(filepath=MODEL_H5)
    receiver_tensors = { #json request input
        "string_array": tf.placeholder(dtype=tf.string,shape=[None]),
    }
    ...
    softmax_outcome = m(features['string_array'])
    predictions = {
        "output_node": softmax_outcome
    }
    ...
    with K.get_session() as sess:
        builder.add_meta_graph_and_variables(sess=sess,
                                            ...
                                            signature_def_map={'classification': signature}, #serving_default is the default of signature_def, otherwise client should alter this setting
                                            legacy_init_op=tf.tables_initializer() #must initialize if using lookup table
                                            )
        builder.save()
    print("H5: {h5path} export to PB: {pbpath} succeed!".format(h5path=MODEL_H5,pbpath=SAVED_PB_PATH))

elif int(args.h5_amount)==2:
    MODEL1_H5 = dirs[0] 
    MODEL2_H5 = dirs[1]
    ...
    with K.get_session() as sess:
        ...
        builder.save()
    print("export to PB: {pbpath} succeed!".format(pbpath=SAVED_PB_PATH))
else:
    print("Dealing with more than 2 h5 models is not completed yet!")
    exit()